...
export default function EditorScreen() {
    ...
    try {
      ...
      await Promise.all(Array.from(scheduledPlatforms).map(async (p) => {
        ...
        const result = await saveScheduledPost({
          ...
          platform: dbPlatform,
          video_url: videoUrl,
          title: videoTitle || snap.hook?.text || 'SmartReel video',
          caption: caption,
          hashtags: hashtags,
          hook_text: hookText,
  ...
  const handleTikTokPublish = async () => {
    ...
    setUploadingToStorage(true);
    const title = videoTitle || snap.hook?.text || 'SmartReel video';
  ...
  const handleYouTubePublish = async () => {
    ...
    const title = videoTitle || snap.hook?.text || 'SmartReel Short';
