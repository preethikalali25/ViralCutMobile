...
RCT_EXPORT_METHOD(combineMediaToVideo:(NSArray<NSDictionary *> *)mediaItems
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        const float maxVSec = 15.0f;  // max seconds taken from each video clip
        ...
        for (NSDictionary *item in mediaItems) {
            ...
            // ── VIDEO ────────────────────────────────────────────────────────
                ...
                // AVAssetImageGenerator: appliesPreferredTrackTransform handles rotation automatically
                AVAssetImageGenerator *gen = [AVAssetImageGenerator assetImageGeneratorWithAsset:asset];
                gen.appliesPreferredTrackTransform = YES;
                gen.maximumSize = CGSizeMake(sz.width * 2, sz.height * 2); // over-sample then downscale
