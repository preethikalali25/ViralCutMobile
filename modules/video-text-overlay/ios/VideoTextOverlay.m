...
// Run the AVFoundation work on a background thread so we never block the main queue.
...
#pragma clang diagnostic pop
    ...
    // Final display size after applying rotation metadata
    ...
    // --- Text layer (Core Animation: y=0 at BOTTOM) ---
    ...
    AVMutableVideoCompositionLayerInstruction *li =
      [AVMutableVideoCompositionLayerInstruction videoCompositionLayerInstructionWithAssetTrack:cv];
...
RCT_EXPORT_METHOD(burnText:(NSString *)videoUri
  ...
  if ([videoUri hasPrefix:@"ph://"]) {
    ...
    if (!asset || asset.mediaType != PHAssetMediaTypeVideo) { resolve(videoUri); return; }
    ...
    [[PHImageManager defaultManager]
       requestAVAssetForVideo:asset options:opts
